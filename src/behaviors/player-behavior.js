...
export class PlayerBehavior extends BaseBehavior {
    ...
    shootProjectile(world) {
        if (!canAttack(this.entity)) return;

        const [dx, dy] = getDirectionOffset(this.entity.direction);
        const x = this.entity.x + dx;
        const y = this.entity.y + dy;

        const projEntity = new Entity(x, y, { speed: 1 });
        const projectile = new Projectile(projEntity, this.entity.direction, this.entity);
